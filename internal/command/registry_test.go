...
func TestRegistryDispatchStringCommands(t *testing.T) {
	...
	assertReply(t, registry.Dispatch(context.Background(), st, nil, []string{"CAS", "name", "wrong", "new"}), ":0\r\n")
	assertReply(t, registry.Dispatch(context.Background(), st, nil, []string{"CAS", "name", "kill", "new"}), ":1\r\n")
	assertReply(t, registry.Dispatch(context.Background(), st, nil, []string{"GET", "name"}), "$3\r\nnew\r\n")
	assertReply(t, registry.Dispatch(context.Background(), st, nil, []string{"SETNXEX", "lock", "token", "60"}), ":1\r\n")
	assertReply(t, registry.Dispatch(context.Background(), st, nil, []string{"SETNXEX", "lock", "other", "60"}), ":0\r\n")
	assertReply(t, registry.Dispatch(context.Background(), st, nil, []string{"GETSETEX", "lock", "new-token", "60"}), "$5\r\ntoken\r\n")
	...
	assertReply(t, registry.Dispatch(context.Background(), st, nil, []string{"RULE", "SET", "session:", "ttl", "60"}), "+OK\r\n")
	assertReplyContains(t, registry.Dispatch(context.Background(), st, nil, []string{"RULE", "LIST"}), "session:")
